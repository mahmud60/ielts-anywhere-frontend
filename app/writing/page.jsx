      ...
      <div style={{
        ...
        <div style={{ flex: 1, minWidth: 220 }}>
          ...
          <p style={{ fontSize: 14, lineHeight: 1.55, opacity: .92, margin: 0, maxWidth: 520 }}>
            Task 1 &amp; Task 2 with AI grading across all four IELTS criteria and detailed feedback.
          </p>
        </div>
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          {[{ icon: <Clock size={12} />, label: "Duration", value: "60 min" }, { icon: <Sparkles size={12} />, label: "Feedback", value: "AI graded" }].map((f) => (
      ...
      {tests.length === 0 ? (
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(290px,1fr))", gap: 16 }}>
          {tests.map((t, i) => {
            ...
            const start = () => { if (isPro) { setStarting(t.id); router.push(`/writing/${t.id}`); } else router.push("/pricing"); };
            return (
              <div key={t.id} className="da-pcard" onClick={() => !isStarting && start()} style={{ padding: 0, overflow: "hidden", opacity: isStarting ? 0.65 : 1 }}>
                ...
                <div style={{ padding: 20, display: "flex", flexDirection: "column", flex: 1 }}>
                  ...
                  <div style={{ fontSize: 16, fontWeight: 700, color: "#0f172a", marginBottom: 5, lineHeight: 1.35 }}>{t.title}</div>
                  <p style={{ fontSize: 13, color: "#64748b", margin: "0 0 14px" }}>{t.test_type === "academic" ? "Academic Writing" : "General Training Writing"}</p>
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 7, marginBottom: 18, marginTop: "auto" }}>
                    <span style={{ display: "inline-flex", alignItems: "center", gap: 5, fontSize: 11.5, fontWeight: 500, color: "#475569", background: "#f4f5f9", borderRadius: 8, padding: "5px 10px" }}>
                      <FileText size={12} /> {t.task_count ?? 2} task{t.task_count !== 1 ? "s" : ""}
                    </span>
                  </div>
          ...
          <div className="da-card" style={{ overflow: "hidden" }}>
            <div style={{ overflowX: "auto" }}>
              <table className="da-table">
                <thead>
                  <tr><th>Test</th><th>Date</th><th>Status</th><th>Band</th><th style={{ textAlign: "right" }}>Report</th></tr>
                </thead>
                <tbody>
                  {history.map((a) => (
