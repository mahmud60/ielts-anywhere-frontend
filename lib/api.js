...
export const api = {
  ...
  linkReferral: (code) =>
    apiFetch(`/auth/link-referral?ref_code=${encodeURIComponent(code)}`, { method: "POST" }),
  ...
  // Listening
  // Not cached: the response now includes the current user's best band per test.
  getListeningTests: () => apiFetch("/listening/tests"),
  getListeningTest: (testId) => apiFetch(`/listening/tests/${testId}`),
  getListeningAttempts: () => apiFetch("/listening/attempts"),
  getListeningAttempt: (attemptId, lang = "en") => apiFetch(`/listening/attempts/${attemptId}?lang=${lang}`),
  ...
  // Reading
  // Not cached: the response now includes the current user's best band per test.
  getReadingTests: () => apiFetch("/reading/tests"),
  ...
  pollWritingAttempt: (attemptId, lang = "en") =>
    apiFetch(`/writing/attempts/${attemptId}?lang=${lang}`),

  // Speaking (LiveKit pipeline)
  lkGetToken: () => apiFetch("/speaking/lk-token", { method: "POST" }),
  submitSpeaking: (transcript, roomName, testSessionId = null) =>
  ...
  getDashboard: (lang = "en") => apiFetch(`/dashboard?lang=${lang}`),

  // Learn (Pro only)
  getVocabularyWords: ({ module, topic } = {}) => {
    const params = new URLSearchParams({ limit: "1000" });
    if (module && module !== "All") params.set("module", module);
    if (topic && topic !== "All") params.set("topic", topic);
    return apiFetch(`/learn/vocabulary/words?${params}`);
  },
  getVocabularyExercises: (lang = "en") => apiFetch(`/learn/vocabulary?lang=${lang}`, { method: "POST" }),
  getGrammarExercises: (lang = "en") => apiFetch(`/learn/grammar?lang=${lang}`, { method: "POST" }),
  ...
  admin: {
    ...
    getAiUsage: () => apiFetch("/admin/ai-usage"),
    getTestAnalytics: () => apiFetch("/admin/test-analytics"),
    getUserAnalytics: () => apiFetch("/admin/user-analytics"),
