...
import { getCachedProfile, setCachedProfile, isProUser } from "./landingAccess";
    ...
    api
      ...
      .then((d) => {
        if (!cancelled) {
          setProfile(d);
          setCachedProfile(d);
          // Enrich the identified person so funnels can segment by plan.
          analytics.setUserProps({
            is_pro: isProUser(d),
            plan: d?.subscription || d?.plan || "free",
          });
        }
      })
      .catch(() => {})
